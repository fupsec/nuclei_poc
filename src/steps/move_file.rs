...
/// 分类归档 + 预过滤（统一入口）。
///
/// argv: [prog, community_path, source_of_truth, output_path, poc_dir]
pub fn run(argv: Vec<String>) -> anyhow::Result<()> {
    let cmap = category::category_map();
    let community_path = argv.get(1).map(|s| s.as_str()).unwrap_or("clone-templates");
    let source_of_truth = argv
        .get(2)
        .map(|s| s.as_str())
        .unwrap_or("clone-templates/projectdiscovery/nuclei-templates");
    let output_path = argv.get(3).map(|s| s.as_str()).unwrap_or("tmp");
    let poc_dir = argv.get(4).map(|s| s.as_str()).unwrap_or("poc_all");
